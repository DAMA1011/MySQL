...
INSERT INTO products (p_name, department, price, weight)
VALUES
...
select paid, count(*) -- p.142
from orders
group by paid
having count(*);

select first_name, last_name, paid -- p.143
from orders
join users on users.id = orders.user_id;

-- order by
select * from products order by price;
select * from products order by price asc; -- ascending: 由小到大，預設值可以不寫
select * from products order by price desc; -- descending: 由大到小

select * from products order by p_name; -- 字母由 A 到 Z
select * from products order by p_name desc; -- 字母由 Z 到 A

select * from products order by price, weight; -- 依序做排序的條件
select * from products order by price, weight desc; -- desc 只會影響單一條件
...
select * from users limit 999; -- 超過總數不會報錯

-- offset
select * from users limit 40 offset 8; -- 9 ~ 48
select * from users limit 40 offset 20; -- 超過總數不會報錯

-- limit 進階用法
select * from users limit 40, 3; -- 41 ~ 43
select * from users limit 3 offset 40; -- 41 ~ 43

-- subquery 子查詢
select p_name, price 
from products 
where price > (select max(price) from products where department = 'toys'); -- select 中的 select，先執行括號內條件

-- distinct
select distinct department from products; -- 過濾資料中重複的部分
select count(distinct department) from products;

select distinct department, p_name from products;
select count(distinct department, p_name) from products;

-- like 模糊搜尋
select * from products where p_name like '%ball%';
select * from products where p_name like '%ball'; -- ball 結尾
select * from products where p_name like 'ball%'; -- ball 開頭

select * from products where price like '___'; -- 幾個 _ 代表幾位數
select * from products where price like '__';
select * from products where price like '_';

select * from products where price like '__' and weight like '_'; -- 用 and 連接多個搜尋
...
u_acc varchar(50) unique,
u_pwd varchar(50)
);
desc user_account;
insert into user_account(u_acc, u_pwd)
values('john', '999');
...
-- (not) in
select * from products where department in ('toys', 'movies'); -- 只有這兩個的資料
select * from products where department = 'toys' or department = 'movies'; -- 要用 or
...
case -- 相當於 if,else 寫法，需在 select 給予一個逗號位置
	when price >= 700 then '高級商品'
	when price <= 300 then '特價商品'
    else '一般商品'
end as '商品種類'
from products;

-- variable
select @myName := 'john'; -- 賦值，:=
select @myName; -- 呼叫

select @myName := 'Tom', @myNumber := 100;
select @myName, @myNumber;

set @myName = 'Kevin', @myNumber = 50; -- =
select @myName, @myNumber;

set @mydatetime = current_timestamp; -- 現在時間
select @mydatetime;

set @mydatetime = now(); -- 現在時間，是函數需括號，常用
select @mydatetime;
...
drop procedure paid_condition;

-- 帶參數的預存程序
DELIMITER //
create procedure productID_department(in productID int) -- in: 輸入參數
begin
	select id, department from products where id = productID;
end //
DELIMITER ;

call productID_department(5);

drop procedure productID_department;

DELIMITER //
create procedure productID_department2(in productID int, out o_department varchar(50)) -- out: 輸出參數
begin
	select department into o_department from products where id = productID;
end //
DELIMITER ;

call productID_department2(5, @p_location);

select @p_location;

drop procedure productID_department2;